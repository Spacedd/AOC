def getInstruction(input):
    return {"direction": input[0], "distance":int(input[1:])}
...
def plot(instructions):
    ...
    for instruction in instructions:
        coord = getInstruction(instruction)
        for i in range(coord["distance"]):
            if coord["direction"] == "R":
                position["x"] += 1
            elif coord["direction"] == "L":
                position["x"] -= 1
            elif coord["direction"] == "U":
                position["y"] += 1
            elif coord["direction"] == "D":
                position["y"] -= 1        
            count += 1
            line.add(str(position["x"]) + "," + str(position["y"]) + "," + str(count))
    return line
...
def getPower(coords, instructions):
    position = {"x":0,"y":0}
    count = 0
    for instruction in instructions:
        coord = getInstruction(instruction)
