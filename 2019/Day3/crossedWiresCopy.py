...
def plot(instructions, signalDiary):
    position = {"x":0,"y":0}
    line = set()
    count = 0
    for instruction in instructions:
        ...
        for i in range(coord["distance"]):
            if coord["direction"] == "R":
                ...
                position["y"] -= 1        
            count += 1
            returnCoord = str(position["x"]) + "," + str(position["y"])
            line.add(returnCoord)
            signal = signalData(returnCoord, count)
            signalDiary.append(signal)
    return line
...
if __name__ == '__main__':
    ...
    signalDiaryB = []
    strongestSignal = 0
    wireOneData = plot(wireOneInstructions, signalDiaryA)
    wireTwoData = plot(wireTwoInstructions, signalDiaryB)
    wireTwoCoords = []
    wireOnePower = 0
    wireTwoPower = 0

    for data in wireOneData:
        if data in wireTwoData:
            for entry in signalDiaryA:
                if entry.getId() == data:
                    wireOnePower = entry.getPower()
            for entry in signalDiaryB:
                if entry.getId() == data:
                    wireTwoPower = entry.getPower()
            signal = wireOnePower + wireTwoPower
